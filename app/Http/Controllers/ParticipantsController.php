...
class ParticipantsController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    {
        //Redirect if Training ID not specify
        if (!isset(request()->id) or !Training::where('id', request()->id)->exists()) {
            redirect(route('voyager.pelatihan.index'))->send();
        }
    }
    ...
    {
        // Convert Training ID to INT
        $request->training_id = (int)$request->training_id;
    ...
    {
        return view('vendor.voyager.peserta.import');
    }
    ...
    {
        //Get Training ID
        // $participant = Participant::select('training_id')->where('id', $id)->first();
