...
class SearchCertificateController extends Controller
{
    ...
    {
        $certificates = Participant::select('participants.id', 'nama_pelatihan', 'pelatihan_tanggal_mulai', 'status', 'no_sertifikat', 'sertifikat')->join('trainings', 'training_id', 'trainings.id')->where('nidn', request()->nidn)->get();
        ...
        if ($certificates->isEmpty()) {
            ...
            foreach ($certificates as $certificate) {
                $url = (empty($certificate->sertifikat)||!isset(json_decode($certificate->sertifikat)[0]->download_link)) ? "" : '<a href="' . Storage::disk(config('voyager.storage.disk'))->url(json_decode($certificate->sertifikat)[0]->download_link) . '" class="view" title="" data-toggle="tooltip" data-original-title="View">
